...
//				System.out.println(sum(0,N-1,b-1,c-1,0));
	...
	public static long init(int start, int end, int index) {
		...
		int mid=(start+end)/2;
		return tree[index]=init(start, mid, index*2+1)+init(mid+1, end, index*2+2);
	}
	...
	public static void update(int start, int end, int index, long diff, int treeIndex) {
		...
		if(start==end) return;
		int mid=(start+end)/2;
		update(start, mid, index, diff, treeIndex*2+1);
		update(mid+1, end, index, diff, treeIndex*2+2);
		
		return;
	}
	
	public static long sum(int start, int end, int findLeft, int findRight, int index) {
		if(findRight<start||end<findLeft) return 0;
		if(findLeft<=start&&end<=findRight) return tree[index];
		
		int mid=(start+end)/2;
		return sum(start, mid, findLeft, findRight, index*2+1)+sum(mid+1, end, findLeft, findRight, index*2+2);
	}
